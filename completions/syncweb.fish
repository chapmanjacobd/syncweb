...
complete -c syncweb -n "__fish_syncweb_using_subcommand network; and not __fish_seen_subcommand_from create join leave list invite kick events test-relay status" -l json -d 'Emit machine-readable JSON. Each command prints a single JSON object (arrays only inside a named key); streaming commands (stats network --follow) print one JSON object per line (NDJSON)'
...
complete -c syncweb -n "__fish_syncweb_using_subcommand stats; and __fish_seen_subcommand_from network" -l since -d 'Only include transfer events recorded since a unix timestamp or period (e.g. 24h, 7d)' -r
...
complete -c syncweb -n "__fish_syncweb_using_subcommand stats; and __fish_seen_subcommand_from network" -l follow -l watch -d 'Stream sync progress and network events live (one JSON object per line under --json)'
complete -c syncweb -n "__fish_syncweb_using_subcommand stats; and __fish_seen_subcommand_from network" -l once -d 'With --follow: print the current snapshot and exit instead of streaming'
