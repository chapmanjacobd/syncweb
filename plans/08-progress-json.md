...
Depends on: plan 01, landed (metadata-first `ls`/`find`/`sort` shipped the
`{folder, path, entries:[...]}` envelope this plan's contract extends; the
remote dimension is proven), plan 02 (eager join
