...
fn subscribe_params_from_filters_maps_flags_to_runtime_filters() -> anyhow::Result<()> {
    ...
    let ignore_self_params =
        SubscribeParams::from_filters(&SubscribeFilters::new(false, true, None, None, None, None))?;
    ensure!(
        ignore_self_params.ignore_session.is_some(),
        "--ignore-self should set a session id"
    );
    ...
    let ingest_only_params =
        SubscribeParams::from_filters(&SubscribeFilters::new(true, false, None, None, None, None))?;
    ensure!(
        ingest_only_params.ingest_only,
        "--ingest-only should enable ingest-only mode"
    );
    ensure!(ingest_only_params.ignore_session.is_none());

    let prefix_params = SubscribeParams::from_filters(&SubscribeFilters::new(
        false,
        false,
    ...
    let glob_params = SubscribeParams::from_filters(&SubscribeFilters::new(
        false,
        false,
        None,
        Some("*.txt".to_owned()),
    ...
    let count_area = count_params
        ...
        count_area.max_count == 3,
        "--max-count should be 3, got {}",
        count_area.max_count
    );
    ensure!(count_area.max_size == 0);

    let size_params =
        SubscribeParams::from_filters(&SubscribeFilters::new(false, false, None, None, None, Some(2048)))?;
    let size_area = size_params
        .area_of_interest
        .as_ref()
        .context("--max-size should set an area of interest")?;
    ensure!(
        size_area.max_size == 2048,
        "--max-size should be 2048, got {}",
        size_area.max_size
    );
